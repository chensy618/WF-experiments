...
Mirrors run_fcn3_cds_station_wind_one_week.py so both models are evaluated on
the same footing:
  - 6-hourly inits: 00:00, 06:00, 12:00, 18:00 UTC each day
  - nsteps = 1  →  6 h lead time only
  - Output: compact zarr (station × time × lead_time), no global grid kept

Total GC forward passes per week: 28 inits × 1 step = 28
(same as the old config: 7 inits × 4 steps = 28)
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run GraphCast Operational with 6-hourly cycling initialization "
            "and extract station-level 10 m wind speed."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Number of 6-hourly forecast steps. Default 1 = 6 h lead time.",
    )
    parser.add_argument(
...
def extract_station_wind(
    ...
    Returns the station Dataset and the (possibly newly built) stn_idx map.
    """
    ...
    if stn_idx is None:
        print("  Building station→grid-index map:")
        stn_idx = _build_stn_idx(lats, lons)

    init_times = _decode_times(g)
    lead_hours  = _decode_lead_hours(g)

    # u10m / v10m: shape (n_init, n_lead, lat, lon)
    u10m = g["u10m"][:]
    v10m = g["v10m"][:]
    ws   = np.sqrt(u10m ** 2 + v10m ** 2).astype(np.float32)
    ...
    for st in STATIONS:
        ...
        # ws_stn: (n_init, n_lead)
        ws_stn = ws[:, :, lat_i, lon_i]
...
def main() -> None:
    ...
    tmp_dir = output_dir / "tmp_gc_raw"
    tmp_dir.mkdir(parents=True, exist_ok=True)

    start_tag = week_start.strftime("%Y%m%d")
    end_tag   = week_end.strftime("%Y%m%d")
    out_zarr  = output_dir / f"graphcast_station_wind_{start_tag}_{end_tag}.zarr"

    print("=" * 70)
    print("GraphCast Operational – 6-hourly station wind")
    print(f"Week : {week_start.date()} → {week_end.date()}")
    print(f"nsteps: {args.nsteps}  ({args.nsteps * 6} h lead)")
    ...
    for day_offset in range(args.days):
        ...
        raw_zarr = tmp_dir / f"gc_raw_{day_tag}.zarr"
        if raw_zarr.exists():
            shutil.rmtree(raw_zarr)

        print(f"\n[Day {day_offset + 1}/{args.days}] {day.date()}  inits: {day_times}")

        io = ZarrBackend(str(raw_zarr))
        deterministic(day_times, args.nsteps, model, data, io)
        ...
        if not args.keep_raw:
            shutil.rmtree(raw_zarr)
            print(f"  Deleted temp zarr: {raw_zarr.name}")

    print("\nCombining daily outputs ...")
    ...
    combined.attrs.update(
        ...
        description=(
            ...
            "6-hourly cycling initialization, CDS ERA5 input, 6 h lead time."
        ),
    )

    print(f"Saving to {out_zarr} ...")
    combined.to_zarr(out_zarr, mode="w")
    (out_zarr / "_SUCCESS").write_text(
